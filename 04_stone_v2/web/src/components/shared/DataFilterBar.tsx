...
interface DataFilterBarProps {
  ...
  /** Whether to show the Trades limit dropdown. Default: true. Set false for Positions. */
  showTradesLimit?: boolean
  /** Exchange names from settings (e.g. ["Binance"]). Merged with trade-derived exchanges. */
  configuredExchanges?: string[]
}

function getAccountOptions(trades: ClosedTrade[], configuredExchanges?: string[]): string[] {
  const tradeExchanges = trades.map((t) => t.exchange).filter(Boolean)
...
export function DataFilterBar({
  ...
  const accountOptions = useMemo(
    () => getAccountOptions(closedTrades, configuredExchanges),
  ...
  return (
    <div className="flex flex-nowrap items-center gap-3 overflow-x-auto pb-1">
      ...
      {/* Last N Trades — hidden when showTradesLimit is false */}
      {showTradesLimit && (
        <div className="flex items-center gap-1.5 shrink-0">
