...
# Service Worker syntax (NOT ES module) - matches the existing worker format
worker_js = r"""// 筹码筛选 Worker (Service Worker)
...
async function handleApiData() {
  const raw = await MARKET_DATA.get('data');
  const updated = await MARKET_DATA.get('last_updated');
...
async function handleDashboard() {
  const kvHtml = await MARKET_DATA.get(KV_HTML_KEY);
...
async function handleStatus() {
  const raw = await MARKET_DATA.get('data');
  const updated = await MARKET_DATA.get('last_updated');
  const count = await MARKET_DATA.get('count');
  return json({ project: '\u7b79\u7801\u7b5b\u9009 \u00b7 \u4ee3\u5e01\u7b5b\u9009\u5668', ok: !!raw, coins: parseInt(count || '0'), updated });
}
"""

# Escape worker JS for JSON string
worker_js_json = json.dumps(worker_js)

# Build the cloudflare_execute code following the working example pattern
code = '''async () => {
  const ACCOUNT = "@A@";
  const KV = "@K@";
  const NAME = "@N@";
  ...
  const metadata = {
    ...
    bindings: [
      { type: "kv_namespace", name: "MARKET_DATA", namespace_id: KV },
    ],
  };
  ...
  return await cloudflare.request({
    method: "PUT",
    path: `/accounts/${ACCOUNT}/workers/scripts/${NAME}`,
    body,
    contentType: "multipart/form-data; boundary=" + b,
    rawBody: true,
  });
}'''.replace("@A@", ACCOUNT_ID).replace("@K@", KV_ID).replace("@N@", WORKER_NAME).replace("@W@", worker_js_json)

out = os.path.join(BASE_DIR, "__cf_step2_v2.txt")
with open(out, "w", encoding="utf-8") as f:
    f.write(code)
print(f"Step 2 v2 script: {len(code)} chars, worker JS: {len(worker_js)} chars")
