...
# JSON-escape the worker JS
worker_js_escaped = json.dumps(worker_js)
...
code = '''async () => {
  ...
  const p1 = enc.encode("--" + b + "\\r\\nContent-Disposition: form-data; name=\\"metadata\\"\\r\\nContent-Type: application/json\\r\\n\\r\\n");
  const p2 = enc.encode(mdJson);
  const p2b = enc.encode("\\r\\n--" + b + "\\r\\nContent-Disposition: form-data; name=\\"_worker.mjs\\"\\r\\nContent-Type: application/javascript+module\\r\\n\\r\\n");
  const p3 = enc.encode(workerJS);
  const p4 = enc.encode("\\r\\n--" + b + "--\\r\\n");

  const len = p1.byteLength + p2.byteLength + p2b.byteLength + p3.byteLength + p4.byteLength;
  const body = new Uint8Array(len);
  body.set(p1, 0); let o = p1.byteLength;
  body.set(p2, o); o += p2.byteLength;
  body.set(p2b, o); o += p2b.byteLength;
  body.set(p3, o); o += p3.byteLength;
  body.set(p4, o);
...
}'''.replace("@A@", ACCOUNT_ID).replace("@K@", KV_ID).replace("@N@", WORKER_NAME).replace("@W@", worker_js_escaped)

out = os.path.join(BASE_DIR, "__cf_step2.txt")
with open(out, "w", encoding="utf-8") as f:
    f.write(code)
print(f"Step 2 script: {len(code)} chars, worker JS: {len(worker_js)} chars")
