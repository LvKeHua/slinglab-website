...
async function hRL(req,kv){const k=RELAY_AUTH_KEY,a=req.headers.get('X-Auth-Key');if(!k||a!==k)return json({ok:false,error:'Unauthorized'},401);try{const b=await req.json();if(!b||typeof b!=='object')return json({ok:false,error:'Must be object'},400);const n=new Date().toISOString();await kv.put('exchange_proxy',JSON.stringify({...b,updated:n}));
  ...
  try{
    if(Array.isArray(b.binance)&&b.binance.length>0){
      ...
      hist.gainers=Array.from(bySym.values());hist.updated=n;
...
const leadEvents=[];
for(const ds of daysArr){if(!raw[ds].gainers)continue;for(let k=0;k<raw[ds].gainers.length;k++){const g=raw[ds].gainers[k];const fs=firstSeen[g.base_asset];if(!fs)continue;if(fs<ds){leadEvents.push({base_asset:g.base_asset,first_seen:fs,gain_day:ds,lead_days:(Date.parse(ds)-Date.parse(fs))/86400000,change_24h_pct:g.change_24h_pct,rank:k+1});}}}
const leadUnion={};
...
winDates.forEach((ds,idx)=>{const fr=winFrs[idx];if(!fr)return;try{const p=JSON.parse(fr);const cs=(p.candidates||[]);if(ds===date)dayCands=new Set(cs.map(c=>c.base_asset));for(const c of cs){if(!firstSeen[c.base_asset]||ds<firstSeen[c.base_asset])firstSeen[c.base_asset]=ds;if(c.forward_score!=null&&(bestScore[c.base_asset]==null||c.forward_score>bestScore[c.base_asset]))bestScore[c.base_asset]=c.forward_score;}}catch(e){}});
...
// 标注
const out=gs.map((g,k)=>{const ba=g.base_asset;const fs=firstSeen[ba];const ever=!!fs;const lead=(fs&&fs<date)?Math.round((Date.parse(date)-Date.parse(fs))/86400000):0;
