...
var STATIC_CACHE_TTL_MS = 10 * 60 * 1e3;
var authCache = { value: null, at: 0 };
var settingsCache = { value: null, at: 0 };
var SETTINGS_CACHE_TTL_MS = 60 * 1e3;
...
async function serveStatic(path, kv) {
  ...
  const now = Date.now();
  const hit = staticCache.get(kvKey);
  if (hit && now - hit.at < STATIC_CACHE_TTL_MS) {
  ...
  let buf = null;
  try {
    buf = await kv.get(kvKey, "arrayBuffer");
  } catch (err) {
  ...
  if (!buf) {
    if (!path.startsWith("/api/")) {
      try {
        buf = await kv.get("/index.html", "arrayBuffer");
      } catch (err) {
        if (!isQuotaError(err)) throw err;
      }
    }
  ...
  if (buf) {
    const type = getContentType(kvKey);
    staticCache.set(kvKey, { data: buf, at: now, type });
...
async function getSettings(kv) {
  ...
  try {
    ...
    settingsCache = { value: parsed, at: now };
    return parsed;
  } catch (err) {
    if (!isQuotaError(err)) throw err;
...
async function putSettings(kv, s) {
  ...
  settingsCache = { value: s, at: Date.now() };
}
...
async function getSiteAuth(kv, env) {
  const now = Date.now();
  if (authCache.value && now - authCache.at < 6e4) {
    return authCache.value;
  }
  let raw = null;
  try {
    raw = await kv.get("site_auth", "json");
  } catch (err) {
    if (!isQuotaError(err)) throw err;
  }
  if (raw?.password) {
  ...
  if (env.SITE_PASSWORD) {
    const auth = { password: env.SITE_PASSWORD, failedAttempts: 0, locked: false, lastResetSentAt: 0 };
    authCache = { value: auth, at: now };
    return auth;
  }
  authCache = { value: null, at: now };
  return null;
}
__name(getSiteAuth, "getSiteAuth");
async function saveSiteAuth(kv, env, auth) {
  authCache = { value: auth, at: Date.now() };
