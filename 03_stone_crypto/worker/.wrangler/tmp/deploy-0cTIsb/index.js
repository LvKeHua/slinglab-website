...
async function checkAuth(request, env) {
  ...
  const expected = await hmacSha2563(payload, env.SITE_PASSWORD);
...
function authCookie(payloadSig, maxAgeSec) {
  return `${AUTH_COOKIE}=${payloadSig}; Path=/stone; HttpOnly; Secure; SameSite=Strict; Max-Age=${maxAgeSec}`;
}
...
<style>
  * { box-sizing: border-box; }
  ...
  .err { color: #f87171; font-size: 12px; margin-top: 10px; min-height: 16px; text-align: center; }
</style>
</head>
<body>
  ...
  <script>
    const f = document.getElementById('f'), p = document.getElementById('p'), err = document.getElementById('err');
    f.addEventListener('submit', async (e) => {
      ...
      if (r.ok) { location.href = '/stone/'; }
      else { err.textContent = '\u5BC6\u7801\u9519\u8BEF'; p.select(); }
    });
  <\/script>
</body>
</html>`;
var index_default = {
  async fetch(request, env) {
    ...
    const isAuthExempt = path === "/api/v1/login" || path === "/api/v1/logout" || path === "/api/upload";
    ...
    try {
      if (path === "/api/v1/login" && method === "POST") {
        ...
        if (!timingSafeEqualStr(body?.password ?? "", env.SITE_PASSWORD)) {
          return jsonResp({ ok: false, error: "Invalid password" }, 401, corsHeaders);
        }
        const payload = btoa(JSON.stringify({ exp: Date.now() + AUTH_TTL_MS }));
        const sig = await hmacSha2563(payload, env.SITE_PASSWORD);
