...
> ✅ 两个 API 密钥均通过 `wrangler secret` 设置，代码中仅通过 `typeof CMC_API_KEY !== 'undefined'` 检查是否存在，**无任何硬编码值**。
...
│   ├── fetchCmcData()      → CoinMarketCap (10s 超时, CMC_API_KEY)
│   ├── fetchCoinGeckoData() → CoinGecko 8页并行 (30s 超时)
│   └── refreshData()       → 三源聚合 + 优先级回退
├── API 路由
│   ├── handleApiData()     → GET /api/data
│   ├── handleDashboard()   → GET /  (从 KV 取 HTML)
│   ├── handleRefresh()     → POST /api/refresh
│   ├── handleUpload()      → POST /api/upload (上传数据)
...
> 注意: 代码引用 `CMC_API_KEY` 和 `COINGECKO_API_KEY` 为 Cloudflare Secrets 变量名，
> 非实际密钥值。`screener-upload-2026` 为上传接口共享认证密钥。
