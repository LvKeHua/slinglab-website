...
function crossValidateRatio(cmcRatio, cgRatio) {
  ...
  if (diff > 0.3) {
    return { conflicted: true, cmc_ratio: Math.round(cmcRatio * 1e4) / 1e4, cg_ratio: Math.round(cgRatio * 1e4) / 1e4, discrepancy: Math.round(diff * 100) };
  }
  return null;
}
__name(crossValidateRatio, "crossValidateRatio");
async function refreshData(kv, env) {
  const [binanceResult, bybitResult, okxResult, cmcResult] = await Promise.allSettled([
    fetchBinanceData(),
    fetchBybitData(),
    fetchOKXData(),
    fetchCmcData(env)
  ...
  if (binanceResult.status === "rejected") {
    exchangeDebug.Binance = { error: String(binanceResult.reason || "unknown"), status: "rejected" };
    console.error("Binance failed:", binanceResult.reason);
  } else {
    exchangeDebug.Binance = { tickers: binanceResult.value?.length || 0, status: "ok" };
  }
  if (bybitResult.status === "rejected") {
    exchangeDebug.Bybit = { error: String(bybitResult.reason || "unknown"), status: "rejected" };
    console.error("Bybit failed:", bybitResult.reason);
  } else {
    exchangeDebug.Bybit = { tickers: bybitResult.value?.length || 0, status: "ok" };
  }
  if (okxResult.status === "rejected") {
    exchangeDebug.OKX = { error: String(okxResult.reason || "unknown"), status: "rejected" };
    console.error("OKX failed:", okxResult.reason);
  } else {
    exchangeDebug.OKX = { tickers: okxResult.value?.length || 0, status: "ok" };
  }
  if (cmcResult.status === "rejected") {
    exchangeDebug.CMC = { error: String(cmcResult.reason || "unknown"), status: "rejected" };
  } else {
    exchangeDebug.CMC = { coins: Object.keys(cmcResult.value || {}).length, status: "ok" };
  }
  await kv.put("exchange_debug", JSON.stringify(exchangeDebug)).catch((e) => console.error("KV write exchange_debug failed:", e));
  ...
  const needCg = !lastCg || Date.now() - new Date(lastCg).getTime() > 60 * 60 * 1e3;
  if (needCg) {
    const cgResult = await fetchCoinGeckoData(env).catch((e) => {
      console.error("CoinGecko failed:", e);
      return null;
    });
    cgMap = cgResult;
    if (cgMap) await kv.put("last_cg_fetch", (/* @__PURE__ */ new Date()).toISOString()).catch((e) => console.error("KV write last_cg_fetch failed:", e));
  }
  const exchangeRows = [];
  {
    const seen = {};
  ...
  if (exchangeRows.length > 0 && cmcMap) {
    ...
    const filtered = merged.filter((r) => r.market_cap != null && r.market_cap >= 15e6);
    if (filtered.length > 0) {
      await kvPutSafely(kv, "data", JSON.stringify(filtered));
      await kvPutSafely(kv, "last_updated", (/* @__PURE__ */ new Date()).toISOString());
      await kvPutSafely(kv, "count", String(filtered.length));
      return;
    }
  }
  if (cmcMap) {
    ...
    if (coins.length > 0) {
      await kvPutSafely(kv, "data", JSON.stringify(coins));
      await kvPutSafely(kv, "last_updated", (/* @__PURE__ */ new Date()).toISOString());
      await kvPutSafely(kv, "count", String(coins.length));
      return;
    }
  }
...
async function fetchBybitData() {
  ...
  try {
    const [instrRes, tickRes] = await Promise.all([
      fetch("https://api.bybit.com/v5/market/instruments-info?category=linear&limit=1000", { signal: controller.signal }),
...
__name(fetchBybitData, "fetchBybitData");
...
__name(fetchBinanceData, "fetchBinanceData");
...
__name(fetchOKXData, "fetchOKXData");
  ...
  if (cmcKey) {
    try {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          return parseCmcResponse(data);
        }
        try {
          const errBody = await res.json();
          console.error("CMC API error:", res.status, errBody);
        } catch (e) {
        }
      } finally {
        clearTimeout(timeout);
...
__name(fetchCmcData, "fetchCmcData");
function parseCmcResponse(data) {
  ...
  for (const coin of data.data) {
    ...
    map[sym.toUpperCase()] = { symbol: sym, market_cap: q.market_cap || null, circulating_supply: circSup, total_supply: totalSup, max_supply: maxSup, circulating_ratio: cr != null ? Math.round(cr * 1e4) / 1e4 : null, cmc_rank: coin.cmc_rank || null, name: coin.name || sym, percent_change_7d: q.percent_change_7d != null ? Math.round(q.percent_change_7d * 100) / 100 : null, price: q.price != null ? Math.round(q.price * 1e4) / 1e4 : null, volume_24h_usdt: q.volume_24h != null ? Math.round(q.volume_24h * 100) / 100 : null };
  }
  return map;
}
__name(parseCmcResponse, "parseCmcResponse");
  ...
  try {
    ...
    for (const result of results) {
      ...
      for (const c of result.value) {
        ...
        let cr = null;
        if (totalSup && totalSup > 0 && circSup != null) cr = circSup / totalSup;
        map[sym] = { symbol: sym, market_cap: c.market_cap || null, circulating_supply: circSup, total_supply: totalSup, max_supply: c.max_supply || null, circulating_ratio: cr != null ? Math.round(cr * 1e4) / 1e4 : null, cmc_rank: c.market_cap_rank || null, name: c.name || sym, percent_change_7d: c.price_change_percentage_7d_in_currency != null ? Math.round(c.price_change_percentage_7d_in_currency * 100) / 100 : null, price: c.current_price != null ? Math.round(c.current_price * 1e4) / 1e4 : null, volume_24h_usdt: c.total_volume != null ? Math.round(c.total_volume * 100) / 100 : null };
      }
    }
...
__name(fetchCoinGeckoData, "fetchCoinGeckoData");
...
__name(assignStars, "assignStars");
...
__name(crunchStars, "crunchStars");
...
__name(unlockLabel, "unlockLabel");
var kvReadCache = /* @__PURE__ */ new Map();
var KV_CACHE_TTL_MS = 60 * 1e3;
function isQuotaError(err) {
  const msg = err instanceof Error ? err.message : String(err);
  return msg.includes("limit exceeded") || msg.includes("10048");
}
__name(isQuotaError, "isQuotaError");
async function kvGetCached(kv, key) {
  const now = Date.now();
  const hit = kvReadCache.get(key);
  if (hit && now - hit.at < KV_CACHE_TTL_MS) return hit.value;
  try {
    const value = await kv.get(key);
    kvReadCache.set(key, { value, at: now });
    return value;
  } catch (err) {
...
__name(kvGetCached, "kvGetCached");
async function kvPutSafely(kv, key, value) {
  try {
    await kv.put(key, value);
  } catch (err) {
    if (!isQuotaError(err)) throw err;
  }
  kvReadCache.set(key, { value, at: Date.now() });
}
__name(kvPutSafely, "kvPutSafely");
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    if (path === "/api/debug-exchange") {
      return handleDebugExchange(kv);
    }
    if (path === "/api/data") {
      return handleApiData(kv);
    }
    if (path === "/api/refresh" && request.method === "POST") {
      return handleRefresh(kv, env);
    }
    if (path === "/api/upload" && request.method === "POST") {
      return handleUpload(request, kv, env);
    }
    if (path === "/api/status") {
      return handleStatus(kv);
    }
  ...
  async scheduled(controller, env, ctx) {
    ...
    ctx.waitUntil((async () => {
      ...
      try {
        ...
        console.error("Scheduled refreshData failed:", e);
      }
...
async function handleApiData(kv) {
  const raw = await kvGetCached(kv, "data");
  const updated = await kvGetCached(kv, "last_updated");
...
async function handleDashboard(kv) {
  const kvHtml = await kvGetCached(kv, KV_HTML_KEY);
...
async function handleRefresh(kv, env) {
  const mem = await kvGetCached(kv, "data");
  console.log("Refresh started, current coins:", mem ? JSON.parse(mem).length : 0);
  await refreshData(kv, env);
  const updated = await kvGetCached(kv, "last_updated");
  const count = await kvGetCached(kv, "count");
...
async function handleUpload(request, kv, env) {
  ...
  try {
    ...
    if (!Array.isArray(body)) {
      return json({ ok: false, error: "Body must be a JSON array of coins" }, 400);
    }
...
async function handleStatus(kv) {
  ...
  const updated = await kvGetCached(kv, "last_updated");
  const count = await kvGetCached(kv, "count");
  return json({ project: "\u7B79\u7801\u7B5B\u9009 \xB7 \u4EE3\u5E01\u7B5B\u9009\u5668", ok: !!raw, coins: parseInt(count || "0"), updated });
}
...
async function handleDebugExchange(kv) {
  const endpoints = [
    ...
    { name: "Binance ws", url: "https://stream.binance.com:9443/ws/btcusdt@ticker" },
    { name: "Bybit ws", url: "https://stream.bybit.com/v5/public/linear" },
  ...
  await kv.put("debug_exchange", JSON.stringify(results)).catch((e) => console.error("KV write debug_exchange failed:", e));
  return json(results);
}
__name(handleDebugExchange, "handleDebugExchange");
export {
