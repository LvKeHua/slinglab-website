...
async function refreshData(kv, env) {
  // -- Tier 1: Every 5 min -- Exchanges + CMC -----------------
  ...
  // Capture exchange debug info for diagnostics
  const exchangeDebug = {};
  ...
  // -- Tier 2: Every 60 min -- CoinGecko ----------------------
  let cgMap = null;
  const lastCg = await kv.get('last_cg_fetch').catch(() => null);
  const needCg = !lastCg || Date.now() - new Date(lastCg).getTime() > 60 * 60 * 1000;
  if (needCg) {
    const cgResult = await fetchCoinGeckoData(env).catch(e => { console.error('CoinGecko failed:', e); return null; });
    cgMap = cgResult;
    if (cgMap) await kv.put('last_cg_fetch', new Date().toISOString()).catch(e => console.error('KV write last_cg_fetch failed:', e));
  }
...
// ── Handler functions (receive KV binding as parameter) ────────
